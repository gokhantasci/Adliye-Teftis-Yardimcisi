...
(() => {
  ...
  async function loadFees() {
    ...
    try {
      ...
      if (!r2.ok)
        throw new Error("Ücret JSON okunamadı (API ve dosya erişilemedi)");
  ...
  function attachScrollReveal(listEl, total, initial = INITIAL_VISIBLE, step = REVEAL_STEP) {
    ...
    listEl.addEventListener("scroll", () => {
      ...
      if (!bottom) return;
      let vis = parseInt(listEl.dataset.visible || "0", 10);
      if (vis >= total) return;
  ...
  function formatCountSmart(n) {
    ...
    if (n < 100_000) return n.toLocaleString("tr-TR");
    if (n < 1_000_000) return (n / 1e3).toFixed(2) + "B";
    if (n < 1_000_000_000) return (n / 1e6).toFixed(2) + "M";
