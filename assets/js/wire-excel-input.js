(() => {
  ...
  function ensureExtraSummaryCard(){
    var host = document.querySelector(".altsag") || document.querySelector(".container .altsag");
    if (!host) return;
    var card = document.getElementById("extraSummary");
    if (!card){
...
const file = e.target.files && e.target.files[0];
    ...
    try{
      ...
      var __delay = 1500 - (Date.now() - __t0); if (__delay>0) { await new Promise(r=>setTimeout(r,__delay)); }
      hideExcelSpinner();
